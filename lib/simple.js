...
/*exports.smsg = (conn, m, hasParent) => {
    ...
    m.reply = async (text, chatId, options) => {
        ...
        conn.reply(chatId ? chatId : m.chat, text, m, { contextInfo: { mentionedJid: conn.parseMention(text), externalAdReply: { title: 'F.A BOTZ MD', body: wm, sourceUrl: 'https://instagram.com/fahrul_mt', thumbnail: data }}, options })
    }
    m.name = m.pushName || conn.getName(m.sender)
